...
{
    public partial class SuperAdminLoginForm : Form
    {
        string user = " ", pass = " ";
        ...
        private void roundedButton1_Click(object sender, EventArgs e)
        {
            ...
            {
                con.Open();
                string cm = "Select Username, Password from account_table WHERE Username=@Username AND Password=@Password AND Number=1";
                ...
                if (count == 1)
                {
                    if (user == tbUsername.Text && pass == tbPassword.Text)
                    {
                        ...
                        SuperAdminDashboardForm dashboardForm = new SuperAdminDashboardForm();
                        dashboardForm.Show();
                        this.Close();
                    }
                    else 
                    {
                        string cmm = "INSERT into acclogserror_table (Username,Log_Date) VALUES('" + "SuperAdmin" + "','" + DateTime.Now.ToString("MM/dd/yyyy") + "')";
                        var cmds = new MySqlCommand(cmm, con);
                        cmds.ExecuteNonQuery();
